...
from particles.particles import collectors, hilbert
from particles.particles import resampling as rs
from particles.particles import rqmc, smoothing, utils
...
class SMC:
    ...
    def __init__(
        ...
        if rng is None:
            ...
            rng = np.random.default_rng(seed=seed)
        self.rng = rng
        print("Local SMC with rng: ", self.rng)
    ...
    def resample_move(self):
        ...
        if self.rs_flag:  # if resampling
            self.A = rs.resampling(self.resampling, self.aux.W, M=self.N, rng=self.rng)
