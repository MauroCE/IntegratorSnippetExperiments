"""
same as AAJ but here we shuffle the data at each run.
"""
import numpy as np
...
if __name__ == "__main__":
    ...
    for _N in N_list:
        ...
        for i in range(n_runs):
            ...
            res = {'N': _N, 'T': _T}
            # Shuffle the data
            shuffle_ix = np.random.default_rng(seed=seeds[i]).choice(a=208, size=208, replace=False)
            y_shuffled = y[shuffle_ix]
            Z_shuffled = Z[shuffle_ix]
            out = smc_hmc_int_snip(N=_N, T=_T, epsilon=0.1, ESSrmin=0.8, _y=y_shuffled, _Z=Z_shuffled, _scales=scales,
    ...
    # Save data
    with open("results/aal/int_snip_eps01_shuffle.pkl", "wb") as file:
