...
def next_annealing_param_unfolded(gamma, ESSrmin, lvn, lvd, nlps, nlls, T, N, full_output=False):
    ...
    if np.sign(f(gamma)) != np.sign(f(1.0)):
        out = sp.optimize.brentq(f, gamma, 1.0, full_output=full_output)
        if full_output:
            root, info = out
            print("\t\tInfo: ", info.root, info.iterations, info.function_calls, info.converged, info.flag)
        else:
            root = out
        return root  # gamma + sp.optimize.brentq(f, 0.0, 1.0 - gamma)
    else:
        if np.sign(f(gamma)) < 0:
            print("\t\t\tLowESS detected while searching for gamma. Constructing trajectories") if full_output else None
        return 1.0


def next_annealing_param_unfolded_new(gamma, ESSrmin, lvn, lvd, nlps, nlls, T, N, full_output=False):
    """This works specifically for the unfolded algorithm. The idea is that inside f we compute the folded ESS
    but using the quantities from the unfolded algorithm somehow."""
    def f(e):
        # Compute ESS if e is larger than 0.0 (like Chopin)
    ...
    if np.sign(f(gamma)) != np.sign(f(1.0)):
        ...
        if np.sign(f(gamma)) < 0 and np.sign(f(1.0)) < 0:
            print("\t\t\tLowESS detected while searching for gamma. Constructing trajectories") if full_output else None

            def f_with_trajectories(e):
                # Compute ESS if e is larger than 0.0 (like Chopin)
                if e > 0.0:
                    # Compute ESS
                    log_num = (- nlps) + e * (- nlls) + lvn  # (N, T+1)
