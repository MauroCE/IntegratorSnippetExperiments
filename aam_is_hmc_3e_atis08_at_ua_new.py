...
1. We store runtime
2. We set `T = (T + ell_max + 1) / 2 rather than (T - ell_max - 1) / 2`
3. We put a check for very large step sizes to just divide by two. This is based on PM.
"""
import numpy as np
    ...
    while gammas[n-1] < 1.0:
        ...
        if adaptive:
            if np.all(np.array(pms[-1]) <= pm_min):
                ...
                unique_Ts = np.unique(Ts * epsilons)
                if len(unique_Ts) == 1:
                    tau = unique_Ts[0]  # there should be only one value TODO: CHECK
                    verboseprint("\t\t\tPMS<PM_MIN: Ts: ", Ts, " Epsilons: ", epsilons, " tau: ", tau)
                else:
                    raise ValueError("Step sizes are all too large: {} {} {}".format(epsilons, Ts, tau))
            else:
                ...
                if Tmin < lmax < Tmax:  # U-turn detected: integration time is too large (we are doubling back)
                    ...
                    if np.all(Ts == Ts[0]) and np.all(epsilons == epsilons[0]):
                        ...
                        # It is likely that when the step size is large, but not crazy, then the ESS condition above is
                        # not satisfied at all times. That's okay, we are still decreasing epsilon through the U-turn
                        # condition and as the tempering problem gets harder, we'll eventually satisfy the ESS condition
                        ...
                        # give more precise U-turn estimates, so we use that step size multiplied by the longest batch
                        ...
                        if (ess_folded_by_group[-1] < ESSrmin*N/3) and np.all(ess_folded_by_group[:-1] >= ESSrmin*N/3):
                            ...
                            Ts = np.array([
                                Ts[0],
                                int(0.5*Ts[0] + 0.25*Ts[-2] + 0.25*Ts[-1]),  # mid between new largest and old smallest
                            ...
                            # If groups with medium/large epsilon have low ESS, shift largest to be midpoint between
                            # small and medium, and medium to be the midpoint between this new large step size and
                            # the smallest
                            Ts = np.array([
                                Ts[0],
...
if __name__ == "__main__":
    ...
    for eps_ix, _epsilon in enumerate(_epsilons):
        ...
        with open(f"results/aam/eps_ix{eps_ix}_adaptive_100runs_timed.pkl", "wb") as file:
