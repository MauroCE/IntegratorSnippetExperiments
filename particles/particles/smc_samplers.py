...
from particles.particles import resampling as rs
from particles.particles.collectors import Collector
from particles.particles.state_space_models import Bootstrap
from particles.particles.variance_mcmc import MCMC_variance
from particles.particles.utils import setup_rng
...
class ArrayMCMC:
    ...
    def __init__(self, rng=None):
        self.rng = setup_rng(seed=None, rng=rng)
...
class ArrayMetropolis(ArrayMCMC):
    ...
    def step(self, x, target=None):
        ...
        accept = self.rng.uniform(low=0.0, high=1.0, size=x.N) < pb_acc
...
class ArrayRandomWalk(ArrayMetropolis):
    ...
    def proposal(self, x, xprop):
        ...
        arr_prop[:, :] = arr + self.rng.normal(size=arr.shape) @ L.T
        return 0.0
...
class MCMCSequence:
    ...
    def __init__(self, mcmc=None, len_chain=10, rng=None):
        self.mcmc = ArrayRandomWalk(rng=rng) if mcmc is None else mcmc
...
class AdaptiveMCMCSequence(MCMCSequence):
    ...
    def __init__(self, mcmc=None, len_chain=10, adaptive=False, delta_dist=0.1, rng=None):
        super().__init__(mcmc=mcmc, len_chain=len_chain, rng=rng)
...
class FKSMCsampler(particles.particles.FeynmanKac):
    ...
    def __init__(self, model=None, wastefree=True, len_chain=10, move=None, rng=None):
        self.model = model
        ...
        if move is None:
            if wastefree:
                self.move = MCMCSequenceWF(len_chain=len_chain, rng=self.rng)
            else:
                self.move = AdaptiveMCMCSequence(len_chain=len_chain, rng=self.rng)
        else:
            self.move = move
...
class Tempering(FKSMCsampler):
    ...
    def __init__(
        self, model=None, wastefree=True, len_chain=10, move=None, exponents=None, rng=None
    ):
        super().__init__(
            model=model, wastefree=wastefree, len_chain=len_chain, move=move, rng=rng
        )
    ...
    def _M0(self, N):
        x0 = ThetaParticles(theta=self.model.prior.rvs(size=N, rng=self.rng))
...
class AdaptiveTempering(Tempering):
    ...
    def __init__(self, model=None, wastefree=True, len_chain=10, move=None, 
                 ESSrmin=0.5, max_iter=1000, rng=None):
        FKSMCsampler.__init__(
            self, model=model, wastefree=wastefree, len_chain=len_chain, move=move, rng=rng
        )
