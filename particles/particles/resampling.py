...
def resampling(scheme, W, M=None, rng=None):
    if rng is None:
        rng = np.random.default_rng(seed=np.random.randint(low=1, high=10000))
    try:
...
def uniform_spacings(N, rng=None):
    ...
    z = np.cumsum(-np.log(rng.uniform(size=N + 1)))
    return z[:-1] / z[-1]


@resampling_scheme
def multinomial(W, M, rng=None):
    ...
    """
    return inverse_cdf(uniform_spacings(M, rng=rng), W)


def multinomial_iid(W, M=None, rng=None):
    ...
    """
    rng = setup_rng(seed=None, rng=rng)
    A = multinomial(W, M=M, rng=rng)
    rng.shuffle(A)
    return A


def multinomial_once(W, rng=None):
    ...
    but it is faster.
    """
    return np.searchsorted(np.cumsum(W), rng.uniform())


@resampling_scheme
def stratified(W, M, rng=None):
    """Stratified resampling."""
    rng = setup_rng(seed=None, rng=rng)
    su = (rng.uniform(M) + np.arange(M)) / M
...
def residual(W, M, rng=None):
    ...
    if sres > 0:
        A[sip:] = multinomial(res / sres, M=sres, rng=rng)
    return A


@resampling_scheme
@jit(nopython=True)
def ssp(W, M, rng=None):
    """SSP resampling.
...
def killing(W, M, rng=None):
    ...
    killed = rng.uniform(size=N) * W.max() >= W
    nkilled = killed.sum()
    A = np.arange(N)
    A[killed] = multinomial(W, nkilled, rng=rng)
    return A


@resampling_scheme
def idiotic(W, M, rng=None):
    """Idiotic resampling.

    For testing only. DO NOT USE.
    """
    a = multinomial_once(W, rng=rng)
...
class MultinomialQueue:
    ...
    def __init__(self, W, M=None, rng=None):
        self.W = W
    ...
    def enqueue(self):
        ...
        self.A = multinomial(self.W, M=self.M, rng=self.rng)[perm]
